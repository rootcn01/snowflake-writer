...
// Generate markdown for export (Obsidian/Notion compatible) with wiki-links for characters and scenes
export function generateMarkdownExport(project) {
  const { title, steps } = project;
  const labels = ['吸引点', '第一幕', '第二幕', '第三幕', '结局'];

  // Build character name map for wiki-links
  const characterNameMap = {};
  if (steps.characters) {
    steps.characters.forEach(char => {
  ...
  // Build scene name map
  const sceneNameMap = {};
  if (steps.scenes) {
    steps.scenes.forEach((scene, i) => {
      sceneNameMap[scene.id] = scene.name || `场景${i + 1}`;
    });
  }
  ...
  // Step 3: Characters - with wiki-links
  md += `## 人物概括\n\n`;
  if (steps.characters && steps.characters.length > 0) {
    steps.characters.forEach(character => {
      const typeLabels = {
      ...
      const charLink = character.name ? `[[${character.name}]]` : '_待填写_';
      md += `### ${charLink}\n\n`;
      md += `- **类型**: ${typeLabels[character.type] || character.type || '_待填写_'}\n`;
      md += `- **目标**: ${character.goal || '_待填写_'}\n`;
      md += `- **冲突**: ${character.conflict || '_待填写_'}\n`;
  ...
  if (steps.characterDetails && steps.characterDetails.length > 0) {
    steps.characterDetails.forEach(c => {
      const charLink = c.name ? `[[${c.name}]]` : '_';
      md += `### ${charLink}\n\n`;
      md += `- **年龄**: ${c.age || '_'}\n`;
      md += `- **外貌**: ${c.appearance || '_'}\n`;
      md += `- **背景**: ${c.background || '_'}\n`;
      md += `- **欲望**: ${c.desire || '_'}\n`;
      md += `- **恐惧**: ${c.fear || '_'}\n`;
      md += `- **价值观**: ${c.values || '_'}\n`;
      md += `- **关键时刻**: ${c.keyMoment || '_'}\n\n`;
    });
  } else {
    md += `_暂无角色详情_\n\n`;
  }
  ...
  // Step 7: Scenes - with wiki-links
  md += `## 场景清单\n\n`;
  if (steps.scenes && steps.scenes.length > 0) {
    steps.scenes.forEach(scene => {
      const sceneLink = scene.name ? `[[${scene.name}]]` : '_待填写_';
      md += `- ${sceneLink} | POV: ${scene.povCharacter || '_待填写_'} | ${scene.summary || '_待填写_'}\n`;
    });
    md += `\n`;
  } else {
    md += `_暂无场景_\n\n`;
  ...
  if (steps.characterBackstories && steps.characterBackstories.length > 0) {
    steps.characterBackstories.forEach(backstory => {
      const charName = characterNameMap[backstory.characterId] || backstory.characterId || '_待填写_';
      md += `### [[${charName}]] 小传\n\n`;
      md += `${backstory.content || '_待填写_'}\n\n`;
    });
  } else {
  ...
  if (steps.sceneDescriptions && steps.sceneDescriptions.length > 0) {
    steps.sceneDescriptions.forEach(desc => {
      const sceneName = sceneNameMap[desc.sceneId] || desc.sceneId || '_待填写_';
      md += `### [[${sceneName}]] 详细描述\n\n`;
      md += `${desc.content || '_待填写_'}\n\n`;
    });
  } else {
